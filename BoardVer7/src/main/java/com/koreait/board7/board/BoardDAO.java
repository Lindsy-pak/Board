...
public class BoardDAO {
	public static int selPagingCnt(BoardDTO param) {
		...
		sb.append(" SELECT CEIL(COUNT(iboard) / ?)").append(" FROM t_board A ").append(" INNER JOIN t_user B ")
		...
		case 3: // 내용
			...
			break;
		case 4: // 글쓴이
			sb.append(" B.unm LIKE '%").append(param.getSearchText()).append("%'");
			break;
		}

		try {
			con = DBUtils.getCon();
			ps = con.prepareStatement(sb.toString()); // toString : 문자열 반환하는거 StringBuilder를 내뱉을때..? 사용 (전에는 String sql로
														// 했기 때문에)
	...
	public static List<BoardDomain> selBoardList(BoardDTO param) {
		...
		String sql = "SELECT " + " A.iboard, A.title, A.iuser, A.regdt " + " , B.unm AS writerNm " + " FROM t_board A "
		...
		case 4: // 글쓴이
			...
			sql += " WHERE B.unm LIKE '%" + param.getSearchText() + "%' "; // 다른 방법
			break;
		}
	...
	public static BoardDomain selBoard(BoardDTO param) {
		...
		String sql = "SELECT " 
				+ " A.title, A.iuser, A.regdt, A.ctnt " 
				+ " , B.unm AS writerNm " 
				+ " FROM t_board A "
				+ " INNER JOIN t_user B " 
				+ " ON A.iuser = B.iuser "
